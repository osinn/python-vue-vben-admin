    ...
    vxeUI.renderer.add('CellTag', {
      renderTableDefault({ options, props }, { column, row }) {
        ...
        const tagOptions = options ?? [
          { type: 'success', label: '已启用', value: 1 },
          { type: 'error', label: '已禁用', value: 2 },
        ];
      ...
      renderTableDefault({ attrs, props }, { column, row }) {
        ...
        const disabled = computed(() => {
          if (attrs) {
            ...
            // 使用 hasAccessByCodes 判断权限
            return attrs?.hasAccessByCodes
              ? !hasAccessByCodes(attrs?.hasAccessByCodes)
              : false;
          }
          // 默认不禁用
          return false;
