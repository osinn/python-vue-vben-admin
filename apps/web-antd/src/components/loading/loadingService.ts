...
/** @name 单例 */
let singleInstance: SpinInstance | undefined;
...
/** @name 入口函数 */
export function loadingService(options: SpinConfig = {}): SpinInstance {
  ...
  addStyle(intance);
  // 将Spin组件插入文档流中
  (resolved.target! as HTMLElement).append(intance.$el);
...
const resolveOptions = (options: SpinConfig): SpinConfig => {
  ...
  target = isString(options.target)
